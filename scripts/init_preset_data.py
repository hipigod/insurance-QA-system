...
- 逐步建立信任关系"""
    ...
    for role in roles:
        try:
            response = requests.post(f"{BACKEND_URL}/roles/", json=role, timeout=5)
            if response.status_code == 200:
                print(f"✅ 创建角色成功: {role['name']}")
            else:
                print(f"❌ 创建角色失败: {role['name']} - {response.text}")
        except Exception as e:
            print(f"❌ 创建角色出错: {role['name']} - {e}")
...
- 说明定期寿险最适合中青年时期"""
    ...
    for product in products:
        try:
            response = requests.post(f"{BACKEND_URL}/products/", json=product, timeout=5)
            if response.status_code == 200:
                print(f"✅ 创建产品成功: {product['name']}")
            else:
                print(f"❌ 创建产品失败: {product['name']} - {response.text}")
        except Exception as e:
            print(f"❌ 创建产品出错: {product['name']} - {e}")


async def init_dimensions():
    """初始化评分维度"""
    print("\n=== 创建评分维度 ===")
...
请给出0-100的分数，并简要说明评分理由。"""
    ...
    for dimension in dimensions:
        try:
            response = requests.post(f"{BACKEND_URL}/dimensions/", json=dimension, timeout=5)
            if response.status_code == 200:
                print(f"✅ 创建维度成功: {dimension['name']}")
            else:
                print(f"❌ 创建维度失败: {dimension['name']} - {response.text}")
        except Exception as e:
            print(f"❌ 创建维度出错: {dimension['name']} - {e}")


async def main():
    """主函数"""
    print("=" * 50)
    print("保险销售智能陪练系统 - 预设数据初始化")
    print("=" * 50)

    # 检查后端是否运行
    try:
        response = requests.get(f"{BACKEND_URL}/roles/", timeout=2)
        print(f"\n✅ 后端服务正常运行")
    except:
        print(f"\n❌ 后端服务未运行，请先启动后端服务！")
    ...
    print(f"管理后台: http://localhost:5173/#/admin")
